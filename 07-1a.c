...
//
if ((shmid = shmget(key, 102400*sizeof(char), 0666|IPC_CREAT|IPC_EXCL)) < 0) { //резервирую 100КБ
//
...
//
if ((shmid = shmget(key, 102400 * sizeof(char), 0)) < 0) {
...
//
if (new) {
FILE* myfile = fopen("07-1a.c", "r"); // открываю файл на чтение
size_t iterator = 0;
while ((array[iterator] = fgetc(myfile)) != EOF)
{
iterator++;
}
fclose(myfile);
} else { //т.к. нам не важно первый или нет раз открываем прогу
FILE* myfile = fopen("07-1a.c", "r"); // открываю файл на чтение
size_t iterator = 0;
while ((array[iterator] = fgetc(myfile)) != EOF)
{
...
//
printf ("Programm text has been written");
