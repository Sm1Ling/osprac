...
//создаю еще 1 поток, его thid будет отличаться
//основной поток будет выполняться параллельно с вызванным методом mythread
//в подпотоках. Для последовательной работы можно использовать pthread.join()
...
mythid = pthread_self(); //в основном потоге глянем что получилось
...
//
pthread_join(thid1, (void **)NULL); //что мы и делаем. Ждем когда созданные потоки завершат работу
